...
function isValidWalk(walk) {
    ...
    let ns = walk.filter(s => s === 'n');
    let ss = walk.filter(s => s === 's');
    let es = walk.filter(s => s === 'e');
    let ws = walk.filter(s => s === 'w');

    return (walk.length === 10 && ((ns.length === ss.length) && (es.length === ws.length)));
}
